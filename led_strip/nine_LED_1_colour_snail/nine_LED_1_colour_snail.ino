...
void loop() {
  ...
  while (i < LEDCount){
    if (abs((i - LEDCount) - (ledToLight % LEDCount)) < 5) { // Green entry
      value.r = (int) (intensity * exp(-abs((i - LEDCount) - (ledToLight % LEDCount))  / 1.5));
      value.g = 0; 
      value.b = 0; 
    } else if (abs(i - (ledToLight % LEDCount)) < 5) { // Green standard
      value.r = (int) (intensity * exp(-abs(i - ledToLight)  / 1.5));
      value.g = 0; 
      value.b = 0; 
    } else if (abs((i + LEDCount) - (ledToLight % LEDCount)) < 5) { // Green exit
      value.r = (int) (intensity * exp(-abs((i + LEDCount) - (ledToLight % LEDCount))  / 1.5));
      value.g = 0; 
      value.b = 0; 
    } else {
